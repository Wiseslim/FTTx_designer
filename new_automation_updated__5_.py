...
MAX_FAT_PER_FDT = 8
MIN_FAT_PER_FDT = 6  # minimum FATs per FDT; underfilled FDTs borrow from connected roads
...
def _cluster_fdts(fdts: List[FDT],
    ...
    """Balanced geographic clustering of FDTs into equal-sized groups.

    Goal
    ----
    Produce the MINIMUM number of closures needed while keeping every cluster
    geographically compact and within the hard capacity cap.  Each closure
    should be ~70-80% full so zones are well-defined and balanced.

    Algorithm
    ---------
    1. Determine k = ceil(N / target) where target = floor(max * 0.75).
       This gives fewer, fuller clusters — matching the reference where 3
       closures serve ~20 FDTs each rather than 5 half-empty ones.

    2. Run K-Means to find k geographically compact centroids.

    3. Assign each FDT to nearest centroid (standard K-Means assignment).

    4. Rebalance: move border FDTs (closest to the other cluster's centroid)
       from oversized clusters to undersized ones.  Only move an FDT if its
       distance to the receiving cluster centroid is <= max_per_cluster * 30m
       (a generous geographic limit), preventing geographically nonsensical
       moves that destroy zone compactness.
    """
    n = len(fdts)
    if n == 0:
        return []
    ...
    # ── Step 1: target ~75% of cap per closure → fewer, fuller closures ───────
    target_per = max(1, int(max_per_cluster * 0.75))  # e.g. 18 for cap=24
    k = max(2, math.ceil(n / target_per))
    k = min(k, n)

    # ── Step 2: K-Means ───────────────────────────────────────────────────────
    coords = np.array([[f.geometry.x(), f.geometry.y()] for f in fdts], dtype=float)
    ...
    # ── Step 4: rebalance with geographic distance guard ──────────────────────
    # Only move a border FDT if it is within the geographic reach of the
    # receiving cluster — prevents swapping FDTs that are far apart.
    geo_limit = max_per_cluster * 30.0   # generous: 24 * 30 = 720 m

    def _centroid(cl: List[FDT]) -> QgsPointXY:
    ...
    for _ in range(max_iters):
        ...
        big_cluster    = clusters[big_idx]
        small_cluster  = clusters[small_idx]
        small_ctr      = _centroid(small_cluster)

        # Find border FDT in big cluster closest to small centroid
        # AND within geographic reach
        best_fdt  = None
        best_dist = float('inf')
        for fdt in big_cluster:
            d = dist_calc.distance_points(fdt.geometry, small_ctr)
            if d < best_dist and d <= geo_limit:
                best_dist = d
                best_fdt  = fdt

        if best_fdt is None:
            break   # no geographically valid candidate — stop rebalancing

        big_cluster.remove(best_fdt)
        small_cluster.append(best_fdt)

    # ── Final: split any cluster that still exceeds hard cap ─────────────────
    final: List[List[FDT]] = []
    for cl in clusters:
        if not cl:
            continue
...
def compute_closure_service_zones(closures: List[DomeClosure]) -> None:
    """Populate each closure.zone_geometry with a Voronoi cell.

    Uses the closure's ACTUAL placed position (snapped_geometry if available,
    otherwise geometry) so that zone boundaries align with where the closure
    physically sits on the network, not the pre-snap centroid.
    """
    if not closures:
        return

    crs = QgsProject.instance().crs()
    pts_layer = QgsVectorLayer(f"Point?crs={crs.authid()}", "_vor_pts", "memory")
    prov = pts_layer.dataProvider()
    prov.addAttributes([QgsField("idx", QVariant.Int)])
    pts_layer.updateFields()

    feat_list = []
    for idx, c in enumerate(closures):
        # Use snapped position — this is where the closure actually sits
        placed_pt = c.snapped_geometry or c.geometry
        feat = QgsFeature(pts_layer.fields())
        feat.setGeometry(QgsGeometry.fromPointXY(placed_pt))
        feat.setAttribute("idx", idx)
        feat_list.append(feat)
    prov.addFeatures(feat_list)

    env = pts_layer.extent()
    grow_amt = max(env.width(), env.height()) * 0.2 + 100
    env.grow(grow_amt)

    vor_layer = None
    try:
        vor_layer = processing.run(
    ...
    if vor_layer:
        for feat in vor_layer.getFeatures():
            geom = feat.geometry()
            for c in closures:
                placed_pt = c.snapped_geometry or c.geometry
                if geom.contains(QgsGeometry.fromPointXY(placed_pt)):
                    c.zone_geometry = geom
                    break

    # Fallback: generous buffer for any closure that did not get a Voronoi cell
    fallback_buffer = MAX_BOX_TO_BOX_DISTANCE * 3.0
    for c in closures:
        if c.zone_geometry is None or c.zone_geometry.isEmpty():
            placed_pt = c.snapped_geometry or c.geometry
            c.zone_geometry = QgsGeometry.fromPointXY(placed_pt).buffer(
                fallback_buffer, 16)
...
def enforce_closure_zone_membership(closures: List[DomeClosure],
    ...
    """Ensure every FDT is assigned only to the closure whose zone contains it.

    Uses the FDT's snapped_geometry (pole position) if available, falling back
    to the raw geometry centroid.  This is critical because the snapped point
    may sit in a different Voronoi cell than the original centroid.

    Assignment priority:
      1. The closure whose Voronoi zone contains the FDT point.
      2. If no zone contains it, assign to the spatially nearest closure
    ...
    hard_cap = CLOSURE_LARGE_CAPACITY // 2  # 24 FDTs max
    outside_all_zones = []

    for fdt in fdts:
        # Use snapped geometry for containment test — it's the actual placed position
        test_pt = fdt.snapped_geometry or fdt.geometry
        pt_geom = QgsGeometry.fromPointXY(test_pt)
        assigned: Optional[DomeClosure] = None

        # Find which closure's zone contains this FDT
        for closure in closures:
            if closure.zone_geometry and closure.zone_geometry.contains(pt_geom):
                if closure.total_fdt < hard_cap:
                    assigned = closure
                else:
                    logger.error(
                        f"{fdt.name} in zone of {closure.name} but at capacity "
                        f"({closure.total_fdt}/{hard_cap})")
                break

        if not assigned:
            outside_all_zones.append(fdt)
            # Assign to nearest closure by actual distance, with capacity
            available = [(c, dist_calc.distance_points(test_pt, c.snapped_geometry or c.geometry))
                         for c in closures if c.total_fdt < hard_cap]
            if available:
                assigned = min(available, key=lambda x: x[1])[0]
                logger.log(
                    f"{fdt.name} outside all zones; assigning to nearest "
                    f"{assigned.name} by distance")
            else:
                logger.error(
                    f"{fdt.name} outside all zones and all closures at capacity; "
                    f"skipping assignment")

        if assigned:
            assigned.fdts.append(fdt)
            assigned.total_fdt += 1

    if outside_all_zones:
        logger.log(f"WARNING: {len(outside_all_zones)} FDT(s) fell outside all closure zones")
        for fdt in outside_all_zones[:5]:
            logger.log(f"  - {fdt.name}")
        if len(outside_all_zones) > 5:
            logger.log(f"  ... and {len(outside_all_zones) - 5} more")

    # Update capacities and remove empty closures
    for c in closures:
        cap = (CLOSURE_LARGE_CAPACITY if c.total_fdt > CLOSURE_24_CORE_THRESHOLD
               else CLOSURE_SMALL_CAPACITY)
        c.feed_capacity = cap
        max_fdt_for_cap = (cap // 8) * MAX_FDT_PER_8CORE
        logger.log(
            f"{c.name}: {c.total_fdt} FDT(s), capacity={cap} cores "
            f"(max {max_fdt_for_cap})")
        if c.total_fdt > max_fdt_for_cap:
            logger.error(
                f"{c.name} over capacity: {c.total_fdt} FDTs > {max_fdt_for_cap}")

    empties = [c for c in closures if c.total_fdt == 0]
    for c in empties:
        closures.remove(c)
        logger.log(f"Removed empty closure {c.name}")
...
def generate_closures(fdts: List[FDT],
    ...
    """Cluster FDTs and create geographically sensible closures.

    Clustering is performed greedily by proximity with a hard cap of
    CLOSURE_LARGE_CAPACITY//2 FDTs per closure. Each cluster is then
    assigned a closure point snapped to the nearest pole on the road network.
    If no pole is within reach the centroid is projected onto the nearest road.

    Pickup-handling rules:
      * If ``pickup_layer`` is provided, closures reference the nearest pickup
        feature (no pickup features are converted into closures).
      * If no layer is supplied, the script will automatically generate one or
        more pickup points. Closures are clustered spatially (about
        CLOSURES_PER_PICKUP per group) and each cluster's centroid is snapped
        to the nearest road to create a road-accessible pickup location.
      * The generated pickups are stored in ``closure.pickup_geometry`` and
        ``closure.pickup_name`` for downstream routing.

    Once the initial clusters are created we compute a service zone for each
    closure (Voronoi partition) and enforce that each FDT is assigned only to
    the closure whose zone contains it.  This prevents cross‑zone
    interceptions and updates capacities accordingly.
    """
    closures: List[DomeClosure] = []
    if not fdts:
        return []

    max_per_closure = CLOSURE_LARGE_CAPACITY // 2
    ...
    if clusters:
        sizes = [len(c) for c in clusters]
        logger.log(
            f"Initial clustering: {len(clusters)} cluster(s), "
            f"sizes: min={min(sizes)} max={max(sizes)} avg={sum(sizes)/len(sizes):.1f}"
        )

    # prepare optional building index for avoidance
    building_index = None
    building_geoms: Dict[int, QgsGeometry] = {}
    if building_layer:
        building_index = index_mgr.build_index(building_layer, cache_geoms=True)
    ...
    # prepare pole index for closure snapping (same as FDT/FAT snapping)
    pole_index = None
    ...
    # prepare pickup index if provided; generation occurs after closures are built
    pick_idx = None
    pick_geoms = {}
    generated_pickups: List[QgsPointXY] = []
    ...
    for ci, cluster in enumerate(clusters, start=1):
        # ── Closure centroid of the cluster ───────────────────────────────────
        cx = sum(f.geometry.x() for f in cluster) / len(cluster)
        cy = sum(f.geometry.y() for f in cluster) / len(cluster)
        closure_pt = QgsPointXY(cx, cy)
        snapped_pt = None

        # ── Step 1: find road junction nearest to centroid ────────────────────
        # A junction pole is one that lies near the endpoint/start of ≥2 roads.
        # We prefer junction poles because the reference image shows closures
        # sitting exactly where road arms branch outward.
        road_geom = None
        junction_pt = None

        if road_index and pole_index:
            # Collect candidate poles within a search radius
            candidate_pole_ids = pole_index.nearestNeighbor(closure_pt, 20)
            best_junction_pole = None
            best_junction_score = -1   # higher = more junction-like
            best_junction_dist  = float('inf')

            for pid in candidate_pole_ids:
                pgeom = pole_geoms.get(pid)
                if not pgeom:
                    continue
                pole_pt = pgeom.asPoint()
                pole_dist = dist_calc.distance_points(closure_pt, pole_pt)

                # Count how many road endpoints this pole is near (junction score)
                pole_geom_q = QgsGeometry.fromPointXY(pole_pt)
                nearby_road_ids = road_index.nearestNeighbor(pole_pt, 6)
                endpoint_count = 0
                for rid in nearby_road_ids:
                    road = roads.get(rid)
                    if not road or not road.geometry or road.geometry.isEmpty():
                        continue
                    ...
                    if not verts:
                        continue
                    # Check if pole is within 5m of a road start or end vertex
                    for endpoint in [verts[0], verts[-1]]:
                        if dist_calc.distance_points(pole_pt, endpoint) <= 5.0:
                            endpoint_count += 1
                            break

                # Score: junction poles score higher; closer poles tie-break
                score = endpoint_count
                if (score > best_junction_score or
                        (score == best_junction_score and pole_dist < best_junction_dist)):
                    best_junction_score  = score
                    best_junction_dist   = pole_dist
                    best_junction_pole   = pole_pt

            if best_junction_pole:
                snapped_pt = best_junction_pole
                closure_pt = best_junction_pole
                junction_pt = best_junction_pole

        # ── Step 2: fallback — snap to nearest road if no pole found ──────────
        if snapped_pt is None:
            if road_index:
                nearest = road_index.nearestNeighbor(closure_pt, 1)
                if nearest:
                    road = roads.get(nearest[0])
                    if road and road.geometry and not road.geometry.isEmpty():
                        road_geom = road.geometry
            ...
            # Fall back to nearest pole without junction preference
            if pole_index:
                candidate_ids = pole_index.nearestNeighbor(closure_pt, 5)
                best_pole  = None
                best_dist  = float('inf')
                for pid in candidate_ids:
                    ...
                    if not pgeom:
                        continue
                    pole_pt = pgeom.asPoint()
                    d = dist_calc.distance_points(closure_pt, pole_pt)
                    if d < best_dist:
                        best_dist = d
                        best_pole = pole_pt
                if best_pole:
        ...
        # pick feed capacity for this cluster
        cap = CLOSURE_LARGE_CAPACITY if len(cluster) > CLOSURE_24_CORE_THRESHOLD else CLOSURE_SMALL_CAPACITY

        # assign pickup geometry if available
        pickup_pt = None
        pickup_name = None
        if pick_idx:
            nearest = pick_idx.nearestNeighbor(closure_pt, 1)
            if nearest:
                pid = nearest[0]
                ...
                # determine name from feature if possible, fallback to id-based label
    ...
    logger.log(f"Generated {len(closures)} dome closures in {len(clusters)} clusters")
    # if no pickup layer was provided, generate pickup points now that closures exist
    if not pick_idx:
        generated_pickups = _generate_pickup_points(closures, roads, road_index, dist_calc, logger)

    # ------------------------------------------------------------------
    # zone creation and strict membership enforcement (correction rules)
    # ------------------------------------------------------------------
    compute_closure_service_zones(closures)
    enforce_closure_zone_membership(closures, fdts, dist_calc, logger)

    # ── Post-enforcement equalisation pass ────────────────────────────────────
    # After Voronoi zone enforcement some closures may have gained or lost FDTs.
    # Run a second rebalance: move border FDTs from the largest to the smallest
    # closure when the imbalance exceeds 2 FDTs and capacity allows.
    _equalise_closure_fdts(closures, dist_calc, logger)

    # Final balance report
    if closures:
        counts = [c.total_fdt for c in closures]
        logger.log(
            f"Final closure balance: {len(closures)} closure(s) — "
            f"min={min(counts)} max={max(counts)} avg={sum(counts)/len(counts):.1f}"
        )
    for c in closures:
        logger.log(f"  {c.name}: {c.total_fdt} FDT(s)")
